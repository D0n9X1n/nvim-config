...
`,t` creates a new terminal buffer in a horizontal split below the current unlocked editor; from Neo-tree, the header, or another utility window it selects an editor before splitting. The original file stays visible with unsaved edits intact, while Neo-tree and the top header stay in place. The new buffer appears as `Terminal` in Bufferline; select that entry in the header and press `x` to remove the buffer and stop its shell. The existing buffer-close action preserves the split and displays a surviving buffer there. If no unlocked editor exists, the mapping warns without changing the layout. `,t` waits for the mapping timeout because `,tt` shares its prefix. In terminal-input mode, either `Ctrl+[` or `Ctrl+]` returns to Normal mode without stopping the shell/job. Terminals that encode `Ctrl+[` as Escape also make Escape leave terminal-input mode. `*` searches backward and `#` searches forward; both center the result.
